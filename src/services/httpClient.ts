...
export const getApiErrorMessage = (error: unknown, fallback = 'Ocurrio un error al procesar la solicitud.'): string => {
  if (axios.isAxiosError(error)) {
    ...
    if (isRecord(responseData) && typeof responseData.message === 'string' && responseData.message.trim()) {
      const details = isRecord(responseData.details) ? responseData.details : null
      const rootCause = details && typeof details.rootCause === 'string' ? details.rootCause.trim() : ''
      if (rootCause) {
        return `${responseData.message} ${rootCause}`
      }
