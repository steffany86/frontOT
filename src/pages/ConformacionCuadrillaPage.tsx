...
import {
  actualizarEstadoConformacionCuadrilla,
...
const ConformacionCuadrillaPage = () => {
  ...
  // Solo aplica al editar (lapiz) una cuadrilla confirmada: marca e_eliminado=1 al
  // guardar. En la creacion el dato siempre sigue insertandose con e_eliminado=0.
  const [confirmadaEliminado, setConfirmadaEliminado] = useState(false)
  ...
  // En 'confirmadas' se permite entrar a edicion, pero unicamente para cambiar el
  // Estado (ver restrictToEstadoOnly mas abajo, que deshabilita el resto de campos).
  const canEditInActiveTab = activeTab === 'general' || activeTab === 'confirmadas'
  const restrictToEstadoOnly = activeTab === 'confirmadas'
  ...
  const handleOpenDetalle = (row: ConformacionCuadrillaRecord) => {
    ...
    setConfirmadaEliminado(isRowEliminado(sourceRow))
  ...
  const columns: Column<ConformacionCuadrillaRecord>[] = [
    {
      ...
      render: (row) => {
        ...
        const estadoTabla = activeTab === 'general'
          ? 'PENDIENTE'
          : readRecordString(row, RECORD_ESTADO_KEYS, row.estado ?? '').trim().toUpperCase()
        return (
                ...
                <div>
                  {estadoTabla ? (
                    <span className={`inline-flex rounded-md px-2.5 py-0.5 text-[11px] font-extrabold uppercase tracking-[0.08em] ${
                      estadoTabla === 'ACTIVO' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                    }`}>
                      {estadoTabla}
                    </span>
                  ) : null}
...
const findVehiculoConflictRecord = (
  ...
  const buildUpdatePayloadFromRow = (row: EditableRow, currentUserId: number, currentSucursal: string): ConformacionCuadrillaInput => {
    ...
    return {
      ...
      supervisorConfirmo: cleanString(session?.nombre || ''),
  ...
  const handleSubmit = async () => {
    ...
    if (activeTab === 'confirmadas') {
      ...
      // Las cuadrillas confirmadas solo permiten cambiar el Estado (ver
      // restrictToEstadoOnly): el resto de campos del formulario queda deshabilitado,
      // por lo que aqui solo se persiste esa columna en BDControlOrdenes.
      setIsResolvingReassignments(true)
      try {
        await actualizarEstadoConformacionCuadrilla(editingId, normalizeEstadoValue(draftRow.estado), confirmadaEliminado)
      } catch (error) {
        setSubmitError(toApiErrorText(error, 'No se pudo actualizar el estado de la cuadrilla confirmada en BDControlOrdenes.'))
        setIsResolvingReassignments(false)
        return
      } finally {
        setIsResolvingReassignments(false)
      ...
      await finalizeDefinitiveSave(
        [targetKey],
        'Estado de la cuadrilla confirmada actualizado'
      )
      return
    }
          ...
          <div className="flex h-[100dvh] w-screen flex-col overflow-hidden bg-white sm:mx-auto sm:my-6 sm:h-[calc(100dvh-3rem)] sm:w-[calc(100%-3rem)] sm:max-w-[1560px] sm:rounded-3xl sm:border sm:border-slate-200 sm:shadow-2xl">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 sm:px-6 sm:py-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  {isViewMode
                    ...
                    : restrictToEstadoOnly
                      ? 'Modo edicion habilitado: solo se puede modificar el Estado (Activo/Ausente) de esta cuadrilla confirmada.'
              ...
              <div className="flex w-full flex-wrap items-center justify-end gap-2 sm:w-auto">
                ...
                ) : isViewMode || activeTab === 'confirmadas' ? null : (
                ...
                >
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-12">
                    <section className="rounded-2xl border border-slate-200 bg-slate-50 p-3.5 md:col-span-1 xl:col-span-2">
                      ...
                      {restrictToEstadoOnly && !isViewMode ? (
                        <label className="mt-2 flex items-center justify-between gap-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700">
                          <span>Eliminar</span>
                          <input
                            type="checkbox"
                            className="h-5 w-5 rounded border-rose-300 text-rose-600 focus:ring-rose-300"
                            checked={confirmadaEliminado}
                            onChange={(event) => setConfirmadaEliminado(event.target.checked)}
                          />
                        </label>
                      ) : null}
                      {false ? (
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        <label className="flex flex-col gap-1 text-xs text-slate-600 [&>span]:font-semibold [&>span]:text-slate-800 sm:col-span-2">
                          ...
                          <select
                            ...
                            disabled={tecnicosQuery.isLoading || isReadOnlyMode || restrictToEstadoOnly}
                          >
                            <option value="">
                        ...
                        <label className="flex flex-col gap-1 text-xs text-slate-600 [&>span]:font-semibold [&>span]:text-slate-800">
                          ...
                          <select
                            ...
                            disabled={isReadOnlyMode || restrictToEstadoOnly}
                          >
                        ...
                        <div className="sm:col-span-2 mt-1 grid gap-3 border-t border-slate-200 pt-3 sm:grid-cols-2">
                          <label className="flex flex-col gap-1 text-xs text-slate-600 [&>span]:font-semibold [&>span]:text-slate-800">
                            ...
                            <select
                              ...
                              disabled={isReadOnlyMode || restrictToEstadoOnly || !canVerDatosTecnico || !row.idTecnico}
                            >
                              <option value="">
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        <label className="flex flex-col gap-1 text-xs text-slate-600 [&>span]:font-semibold [&>span]:text-slate-800">
                          ...
                          <select
                            ...
                            disabled={isReadOnlyMode || restrictToEstadoOnly || !row.idTecnico || isVehiculosLoading}
                          >
                            <option value="">
                              {!row.idTecnico
                          ...
                          <div className="mt-1 flex items-center gap-2 text-[11px] font-medium text-slate-600">
                            <input
                              ...
                              disabled={isReadOnlyMode || restrictToEstadoOnly || !row.idTecnico}
                            />
                      ...
                      <div className="mt-4 grid gap-3 border-t border-slate-200 pt-4 sm:grid-cols-2">
                        <label className="flex flex-col gap-1 text-xs text-slate-600 [&>span]:font-semibold [&>span]:text-slate-800">
                          ...
                          <select
                            ...
                            disabled={digitadoresQuery.isLoading || isReadOnlyMode || restrictToEstadoOnly}
                          >
                            <option value="">
