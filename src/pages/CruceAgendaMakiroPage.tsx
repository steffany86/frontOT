...
const displayHeaderLabel = (column: string): string => {
  const labels: Record<string, string> = {
    ...
    TECNICO: 'TECNICO',
    tecnico: 'TECNICO',
    Actualizado_NODO_SI_NO: 'ACT. NODO',
    Actualizado_NODO: 'ACT. NODO',
    actualizadoNodo: 'ACT. NODO',
    Actualizado_GEO_SI_NO: 'ACT. GEO',
    Actualizado_GEO: 'ACT. GEO',
    actualizadoGeo: 'ACT. GEO',
    Actualizado_BOLETA_SI_NO: 'ACT. BOLETA',
    Actualizado_BOLETA: 'ACT. BOLETA',
    actualizadoBoleta: 'ACT. BOLETA',
    Actualizado_DIGITACION_SI_NO: 'ACT. DIGITACION',
    Actualizado_DIGITACION: 'ACT. DIGITACION',
...
const CruceAgendaMakiroPage = () => {
  ...
  const [tieneDetalle, setTieneDetalle] = useState('')
  ...
  const tieneDetalleOptions = useMemo(
    () => uniqueOptions(rows, [...columnAliasGroups.tieneDetalle]),
    [rows]
  )
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (!matchesSelectFilter(row, [...columnAliasGroups.tieneDetalle], tieneDetalle)) return false
      if (!term) return true
      return Object.values(row).some((value) => displayValue(value).toLowerCase().includes(term))
    })
  }, [actualizadoBoleta, actualizadoDigitacion, actualizadoGeo, actualizadoNodo, estadoSeleccionados, filtro, rows, sucursal, tecnicoNombre, tieneDetalle, tipoCruce])

  useEffect(() => {
    setPage(1)
  }, [actualizadoBoleta, actualizadoDigitacion, actualizadoGeo, actualizadoNodo, fecha, filtro, estadoSeleccionados, sucursal, tecnicoNombre, tieneDetalle, tipoCruce])
    ...
    <div className="space-y-4 p-4 sm:p-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          ...
          <label className="text-sm font-semibold text-slate-700">
            Tiene detalle
            <select value={tieneDetalle} onChange={(event) => setTieneDetalle(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500">
              <option value="">Todos</option>
              {tieneDetalleOptions.map((value) => (
                <option key={value} value={value}>
                  {value}
                </option>
              ))}
