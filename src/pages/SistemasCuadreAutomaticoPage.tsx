...
const SistemasCuadreAutomaticoPage = () => {
  ...
  const [validacionesSaldo, setValidacionesSaldo] = useState<Record<string, unknown>[]>([])
      ...
      try {
        ...
        if (event.type === 'complete') {
          if (event.resultado) {
            ...
            setValidacionesSaldo(event.resultado.validacionesSaldo ?? [])
          }
          setError(null)
    ...
    <div className="space-y-5 pb-8">
      ...
      >
        <div className="space-y-4">
          ...
          {!processRunning && !error && resultado && validacionesSaldo.length ? (
            <div className="rounded-lg border border-amber-200 bg-amber-50">
              <div className="border-b border-amber-200 px-3 py-2 text-sm font-bold text-amber-900">
                Validaciones de saldo ({validacionesSaldo.length})
              </div>
              <div className="max-h-48 divide-y divide-amber-100 overflow-y-auto">
                {validacionesSaldo.map((validacion, index) => (
                  <pre key={index} className="whitespace-pre-wrap px-3 py-2 text-xs text-amber-950">
