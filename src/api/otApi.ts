...
export type OtMaterialModificable = {
...
export type OtModificableHoy = {
  ...
  estado?: string
  tor?: string
  materiales: OtMaterialModificable[]
  puedeModificar: boolean
  bloqueo?: string
}

const mapOtModificableHoy = (row: UnknownRecord): OtModificableHoy => ({
  ...row,
  idVenta: readNumber(row, ['idVenta', 'Id_Venta', 'id_venta']) ?? 0,
  ordenTrabajo: readString(row, ['ordenTrabajo', 'OrdenTrabajo', 'orden_trabajo']),
  codigoCliente: readNumber(row, ['codigoCliente', 'CodigoCliente', 'codigo_cliente']),
  idRuta: readNumber(row, ['idRuta', 'Id_Ruta', 'id_ruta']),
  idVendedor: readNumber(row, ['idVendedor', 'Id_Vendedor', 'id_vendedor']),
  grupo: readString(row, ['grupo', 'Grupo', 'ruta', 'Ruta']),
  fechaHoraDetalle: readString(row, ['fechaHoraDetalle', 'FechaHoraDetalle', 'fecha_hora_detalle']),
  estado: readString(row, ['estado', 'Estado']),
  tor: readString(row, ['tor', 'TOR', 'Tor']) || undefined,
  materiales: normalizeArrayResponse<UnknownRecord>(row.materiales).map((material) => ({
    ...material,
    idCodigoVenta: readNumber(material, ['idCodigoVenta', 'Id_CodigoVenta', 'id_codigo_venta']) ?? 0,
    idProducto: readNumber(material, ['idProducto', 'Id_Producto', 'id_producto']) ?? 0,
    producto: readString(material, ['producto', 'Producto', 'nombre', 'Nombre']),
    cantidad: readNumber(material, ['cantidad', 'Cantidad']) ?? 0,
    tipoMaterial: readString(material, ['tipoMaterial', 'TipoMaterial']),
    serie: readString(material, ['serie', 'Serie', 'codInicio', 'Cod_Inicio']) || undefined,
    chipId: readString(material, ['chipId', 'ChipID', 'ChipId']) || undefined,
  })),
  puedeModificar: readBoolean(row, ['puedeModificar', 'PuedeModificar']) ?? false,
  bloqueo: readString(row, ['bloqueo', 'Bloqueo']) || undefined,
})
...
export const fetchOtModificablesHoy = async (): Promise<OtModificableHoy[]> => {
  const { data } = await api.get('/ot/modificar-hoy', {
    params: { _ts: Date.now() },
    headers: {
  ...
  const rows = normalizeArrayResponse<UnknownRecord>(data)
  return rows.map(mapOtModificableHoy)
}

export const fetchOtModificableHoy = async (idVenta: number): Promise<OtModificableHoy> => {
  const { data } = await api.get(`/ot/${idVenta}/modificar-hoy`, {
    params: { _ts: Date.now() },
    headers: {
      'Cache-Control': 'no-cache, no-store',
      Pragma: 'no-cache',
  ...
  if (!isRecord(row)) {
    throw new Error('La OT no devolvio un formato valido.')
  }
  return mapOtModificableHoy(row)
}

export const updateOtCantidadesHoy = async (
  idVenta: number,
  materiales: Array<{ idCodigoVenta: number; cantidad: number }>
): Promise<void> => {
  await api.put(`/ot/${idVenta}/cantidades-hoy`, { materiales })
}

export type OtAgregarMaterialPayload = {
  idProducto: number
  idTipoMaterial: number
  cantidad: number
  serie?: string
...
export const agregarMaterialModificarHoy = async (
  idVenta: number,
  payload: OtAgregarMaterialPayload
): Promise<OtModificableHoy> => {
  const { data } = await api.post(`/ot/${idVenta}/modificar-hoy/materiales`, payload)
  const row = unwrapData(data)
  if (!isRecord(row)) {
    throw new Error('La OT no devolvio un formato valido.')
  }
...
export const toIsoDateParam = (value?: string): string => {
  ...
  const dmyLike = raw.match(/^(\d{2})\/(\d{2})\/(\d{4})(?:[ T].*)?$/)
  if (dmyLike) return `${dmyLike[3]}-${dmyLike[2]}-${dmyLike[1]}`

  const ymdLike = raw.match(/^(\d{4})-(\d{2})-(\d{2})(?:[ T].*)?$/)
  if (ymdLike) return `${ymdLike[1]}-${ymdLike[2]}-${ymdLike[3]}`
...
export const registrarVentaParaRegistroOtWb = async (
  ...
  const body: Record<string, unknown> | FormData = (() => {
    ...
    formData.append('pdf', fileToUpload, fileToUpload.name || originalName)
    return formData
  })()
  try {
